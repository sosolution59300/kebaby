...
import com.mycompany.kebaby.config.KafkaProperties;
...
class KebabyKafkaResourceIT {
    ...
    void setup() {
        ...
        KebabyKafkaResource kafkaResource = new KebabyKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/kebaby-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/kebaby-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
